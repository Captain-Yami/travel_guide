...
class FortsMuseumDetails extends StatefulWidget {
  final String locationType;

  const FortsMuseumDetails({super.key, required this.locationType});

  @override
  State<FortsMuseumDetails> createState() => _AddLocationDetailsPageState();
}

class _AddLocationDetailsPageState extends State<FortsMuseumDetails> {
  ...
  Future<String?> _uploadImageToCloudinary() async {
    ...
    // Upload the image to Cloudinary under the 'fortsmuseum' folder
    final response = await cloudinary.upload(
      file: _image!.path,
      resourceType: CloudinaryResourceType.image,
      folder: 'fortsmuseum', // Cloudinary folder name
      fileName: 'fortsmuseum_${DateTime.now().millisecondsSinceEpoch}', // Dynamic file name
    );
  ...
  void _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      if (imageUrl != null) {
        try {
          // Save the fortsmuseum details to Firestore
          await FirebaseFirestore.instance.collection('Places').doc('Locations').collection('FortsMuseum').add({
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('FortsMuseum details added successfully!')));
